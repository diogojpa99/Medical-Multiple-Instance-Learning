...
def get_args_parser():
    ...
    parser.add_argument('--lr_k_decay', type=float, default=1., help='LR k rate (default: 1.)')
    parser.add_argument('--lr_cycle_mul', type=float, default=1., help='LR cycle mul (default: 1.)')
    parser.add_argument('--lr_cycle_decay', type=float, default=.1, help='LR cycle decay (default: 0.1)')
    parser.add_argument('--lr_cycle_limit', type=int, default=1, help= 'LR cycle limit(default: 1)')
    parser.add_argument('--num_steps_per_epoch', type=int, default=10, help='number of steps per epoch (default: 10)')
    parser.add_argument('--num_cycles', type=int, default=2, help='Number of cycles (default: 2)')
    ...
    # MIL parameters
    parser.add_argument('--pooling_type', default='max', choices=['max', 'avg', 'topk'], 
                        type=str, help="")
    parser.add_argument('--mil_type', default='instance', choices=['instance', 'attention', 'embedding'],
                        type=str, help="")
    ...
    # Model EMA parameters (Note: EMA is not used in this project)
    ...
    # Data augmentation parameters (Note: Data augmentation is not used in this project)
...
def main(args):
    ...
    if args.data_path:
        ...
        if args.lr_scheduler:
            if args.sched == 'exp':
                ...
                num_epoch_repeat = args.epochs/args.num_cycles
                args.lr_cycle_limit = num_epoch_repeat+1
                lr_scheduler = CosineLRScheduler(optimizer, 
                                                 t_initial=num_epoch_repeat*args.num_steps_per_epoch,
                                                 lr_min=args.min_lr, 
                                                 cycle_mul=args.lr_cycle_mul,
                                                 cycle_decay=args.lr_cycle_decay,
                                                 cycle_limit=args.lr_cycle_limit,
                                                 warmup_lr_init=args.warmup_lr,
                                                 noise_pct=args.lr_noise_pct,
                                                 noise_std=args.lr_noise_std,
                                                 noise_range_t=(args.start_epoch, 10), # Start adding noise till 10th epoch
                                                 t_in_epochs=False,)
            else:    
    ...
    if args.resume:
        ...
        if args.visualize:
            print('----------------- Visualization -------------------')
            visualization.Visualize_Instance_Scores(model=model, datapath=args.images_path, outputdir=output_dir, args=args)
            #visualization.Visualize_Instance_Scores_Many(model=model, datapath=args.images_path, outputdir=output_dir, args=args)

        elif args.evaluate:
            print('----------------- Evaluation -------------------')
            best_results = engine.evaluation(model=model,dataloader=data_loader_val,criterion=criterion,device=device,args=args)
        ...
        for epoch in range(args.start_epoch, args.epochs + args.cooldown_epochs):
            ...
            num_updates = epoch * args.num_steps_per_epoch

            for i in range(args.num_steps_per_epoch):
                num_updates += 1
